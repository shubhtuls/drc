...
void rpSem_forward(rpSem* rp, double *predsGeom, double* gradPredsGeom, double* predsSem, double* gradPredsSem, double* E_psi, double *origins, double*directions, double* depths, int* classIds, int bs, int nrays){
    ...
    for(int b=0; b < bs; b++){
        for(int r=0; r < nrays; r++){
            ...
            psi_r[N_r] = psi_func_sem(rp->maxDepth, d_r, rp->bgDist, c_r);
            E_psi[b*nrays + r] = psi_r[0];

            for(int ix=0;ix<N_r;ix++){
                 E_psi[b*nrays + r] += (psi_r[ix+1] - psi_r[ix])*prod_x[ix];
                 p_zr_i = ((ix > 0) ? prod_x[ix-1] : 1) - prod_x[ix];
                 gradPredsSem[b*nCells*nClasses + x_r[ix]*nClasses + c_r] += p_zr_i*(-1/predsSem[b*nCells*nClasses + x_r[ix]*nClasses+c_r]);
